...
export default {
  async fetch(request, env, ctx) {
    ...
    const slug = path.slice(1); // Ambil ID setelah slash
...
<head>
    ...
    <meta name="referrer" content="no-referrer"> <!-- NO REFERRER SAFETY -->
    ...
    <style>
        ...
        .action-btn.completed {
            background: #064e3b; border-color: #10b981; cursor: default;
        }
...
<body>
    ...
    <script>
        // TARGET URL (Dikaburkan agar tidak mudah diintip di View Source)
        const _0xTarget = "${encodeURIComponent(data.target)}";
        
        let status = { sub: false, like: false, wa: false };

        function handleClick(type, url) {
            if (status[type]) return; // Kalau sudah selesai, diamkan

            // 1. Buka Tab Baru
            window.open(url, '_blank');

            // 2. Tampilkan status "Checking"
            const msgEl = document.getElementById('msg-' + type);
            msgEl.style.display = 'block';
            
            // 3. Simulasi pengecekan (Timer 8 detik)
            // User "dipaksa" menunggu seolah sistem sedang verifikasi API
            setTimeout(() => {
                markComplete(type);
            }, 5000); 
        }
        ...
        function markComplete(type) {
            ...
            msg.style.display = 'none'; // Sembunyikan teks checking
        ...
        function finalLink() {
            ...
            // Decode URL dan Redirect dengan aman (No Referrer)
            const realUrl = decodeURIComponent(_0xTarget);
            
            // Teknik No-Referrer ganda
            const link = document.createElement('a');
            link.href = realUrl;
            link.rel = 'noreferrer noopener';
            link.target = '_blank'; // Tetap di tab baru agar aman
